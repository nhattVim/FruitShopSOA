...
export const isProductInStock = (productId) =>
    get(`/inventory/inStock/${productId}`);
export const recordInboundInventory = (inventory) =>
    post("/inventory/inbound", inventory);
export const deductStock = (productId, quantity) =>
    post(`/inventory/outbound/${productId}?quantity=${quantity}`);
export const getExpiringItems = (thresholdDate) =>
    get(
        `/inventory/expiring${thresholdDate ? `?thresholdDate=${thresholdDate}` : ""}`,
    );
export const getInventoryDetailsByProductId = (productId) =>
    get(`/inventory/${productId}`);
export const convertUnits = (productId, quantity, fromUnit, toUnit) =>
    get(
        `/inventory/convert?productId=${productId}&quantity=${quantity}&fromUnit=${fromUnit}&toUnit=${toUnit}`,
    );
...
export const updateOrderStatus = (id, status) =>
    put(`/order/${id}/status?status=${status}`);
...
export const addMembershipPoints = (id, points) =>
    put(`/customer/${id}/points?points=${points}`);
export const getPurchaseHistory = (id) => get(`/customer/${id}/history`);

// --- Pricing & Promotion Service API ---
export const setProductPrice = (priceRequest) =>
    post("/pricing/price", priceRequest);
export const getProductPrice = (productId) =>
    get(`/pricing/price/${productId}`);
export const createPromotion = (promotion) =>
    post("/pricing/promotion", promotion);
export const getAllPromotions = () => get("/pricing/promotion"); // Added for completeness
export const applyPromotion = (productId, originalPrice) =>
    get(`/pricing/promotion/apply/${productId}?originalPrice=${originalPrice}`);
...
export const applyVoucher = (voucherCode, orderTotal) =>
    get(
        `/pricing/voucher/apply?voucherCode=${voucherCode}&orderTotal=${orderTotal}`,
    );
...
export const getPaymentStatusByOrderId = (orderId) =>
    get(`/payment/${orderId}`);
export const refundPaymentByOrderId = (orderId) =>
